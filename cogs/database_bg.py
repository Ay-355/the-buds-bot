...
#* Account database commands
    ...
    async def adduser(self, ctx, player_tag, member: discord.Member):
        ...
        try:
            ...
            #! Change their nickname to format we want
            ...
            #! Add clan roles for first time so we don't have to manually do it

            try:

                role_to_give_id = self.bot.dbconn.get_role_from_clan_tag((player.clan.tag, ctx.author.guild.id,))
                role_to_give = ctx.author.guild.get_role(role_to_give_id)
...
#* Gets the registered accounts for a user
    ...
    #! First we have to add the role and clan to the database
    ...
    #! Gets all of the linked roles in a guild
    ...
    @commands.command(name="unlinkrole")
    async def unlinkrole(self, ctx, role: discord.Role):

        try:
            self.bot.dbconn.remove_role_from_role_to_tag((role.id, ctx.author.guild.id,))
            await ctx.send(f"Removed role {role.name}({role.id}) from the database")
        except:
            traceback.print_exc()
